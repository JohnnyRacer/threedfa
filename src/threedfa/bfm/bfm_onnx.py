...
def _load_tri(bfm_fp):
    if osp.split(bfm_fp)[-1] == 'bfm_noneck_v3.pkl':
        tri = _load(make_abs_path(f'{abs_cfg_path}/tri.pkl'))  # this tri/face is re-built for bfm_noneck_v3
    else:
...
class BFMModel_ONNX(nn.Module):
    ...
    def __init__(self, bfm_fp=f'{abs_cfg_path}/bfm_noneck_v3.pkl', shape_dim=40, exp_dim=10):
...
if __name__ == '__main__':
    convert_bfm_to_onnx(f'{abs_cfg_path}/bfm_noneck_v3.onnx')
