...
for test_t in test_types:
    ...
    for i in range(0,5):
        ...
        out_fp = f'./media/test_results/{test_t}_{i+1}.png'
        hash_dict[test_t] = {}
        ...
        for idx, lm in enumerate(landmarks):
            with open(f'./media/test_results/lms/dense_{i}_{idx}.npy', 'wb') as wb:
                np.save(wb ,lm)
        for idx, im in enumerate(plotted_lms):
            plotted_im = f'./media/test_results/lmdense_{i}_{idx}.png'
        ...
        plotted_lms = [FaceUtils.cv_draw_landmark(loaded_img,lm) for lm in landmarks]
        for idx, lm in enumerate(landmarks):
            with open(f'./media/test_results/lms/sparse_{i}_{idx}.npy', 'wb') as wb:
                np.save(wb ,lm)

        for idx, im in enumerate(plotted_lms):
            plotted_im = f'./media/test_results/lmsparse_{i}_{idx}.png'
            Image.fromarray(im).save(plotted_im)

hash_dict['base_hashes'] = {"media/test_images/1.jpeg": "007941a9befa67541921074ae3718113", "media/test_images/2.jpeg": "ae0b40dddba5d8666017ea45243102e5", "media/test_images/3.jpeg": "618c8c96fafc1cbf964f205f9a115ee1", "media/test_images/4.jpeg": "61ab934ad44f079a2c13826a4fad93f7", "media/test_images/5.jpeg": "9e467cf7fd0e543521c647dd4d8e372f"}

print(hash_dict)

with open('./test_hashesRef.json', 'r') as fp:
    loaded_hashes = json.load(fp)

print(f"Hashes are the same {hash_dict == loaded_hashes} ")
